...
class XstsSemantics(KCFGSemantics):
...
class XstsDist:
    ...
    def _find(target: str) -> Path:
        ...
        * otherwise ask `kdist`
        ...
        return kdist.get(f'xsts-semantics.{target}')
...
class KXsts:
    ...
    def prove(
        ...
        *,
        ...
        include_dirs = [self.dist.source_dir / 'xsts-semantics'] + (
        ...
        with self.explore(id=spec_label) as kcfg_explore:
            prover = APRProver(
                ...
                cut_point_rules=['XSTS.while'],
                terminal_rules=['XSTS.done'],
            )
